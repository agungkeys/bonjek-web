...
export async function getStoreDetail(slugData='', search='') {
  const slug = slugData ? JSON.stringify(slugData) : '';
  const q = search ? JSON.stringify(search) : '';
  const data = await fetchAPI(
    `
    query StoreDetail {
      ...
      productCategories(where: {product_every: {store: {slug: "bears-house-belimbing"}, name_contains: ""}}) {
        id
        name
        slug
        product {
